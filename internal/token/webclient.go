...
// ErrTokenWebClient is an error raised by a TokenWebClient web handler.
type ErrTokenWebClient struct {
	Context string
	Err     error
	Msg     string // user facing
}

func (e ErrTokenWebClient) Error() string {
	return fmt.Sprintf("TokenWebClient error: %s: %v", e.Context, e.Err)
}
...
func (twc *TokenWebClient) InitiateLogin(ctx context.Context) (string, error) {
	...
	if _, err := rand.Read(b); err != nil {
		return "", ErrTokenWebClient{
			Context: "in InitiateLogin",
			Err:     fmt.Errorf("failed to generate state: %w", err),
			Msg:     "failed to generate login state",
		}
	}
...
// keys.
func (twc *TokenWebClient) InitiateWebLogin() func(w http.ResponseWriter, r *http.Request) error {
	...
	return func(w http.ResponseWriter, r *http.Request) error {
		...
		if err != nil {
			return ErrTokenWebClient{
				Context: "in InitiateWebLogin",
				Err:     fmt.Errorf("failed to initiate login: %w", err),
				Msg:     "failed to initiate web login",
			}
		}
...
func (twc *TokenWebClient) WebLoginCallBack() func(w http.ResponseWriter, r *http.Request) error {
	...
	return func(w http.ResponseWriter, r *http.Request) error {
		...
		if state == "" {
			msg := "missing 'state' in session"
			return ErrTokenWebClient{
				Context: "in WebLoginCallBack",
				Err:     errors.New(msg),
				Msg:     msg,
			}
		}
		...
		if queryState == "" || queryState != state {
			msg := "missing oauth 'state' in platform response"
			return ErrTokenWebClient{
				Context: "in WebLoginCallBack",
				Err:     errors.New(msg),
				Msg:     msg,
		...
		if verifier == "" {
			msg := "missing pkce 'verifier' in session"
			return ErrTokenWebClient{
				Context: "in WebLoginCallBack",
				Err:     errors.New(msg),
				Msg:     msg,
		...
		if code == "" {
			msg := "missing 'code' in platform response"
			return ErrTokenWebClient{
				Context: "in WebLoginCallBack",
				Err:     errors.New(msg),
				Msg:     msg,
		...
		if tok == nil {
			return ErrTokenWebClient{
				Context: "in WebLoginCallBack",
				Err:     fmt.Errorf("nil token received from oauth2.VerifierOption: %w", err),
				Msg:     "nil token received from token verification",
			}
		}
		...
		if err != nil {
			return ErrTokenWebClient{
				Context: "internal error in WebLoginCallBack",
				Err:     fmt.Errorf("token registration error: %w", err),
				Msg:     "internal token registration error",
			}
		}
