/*
 Reconciler app: SQL
 invoice.sql
 invoice with line items and donation total
...
FROM (
    WITH concrete AS (
        SELECT 'INV-2025-101' AS invoice_number
    )
    ...
    ,reconciled_donations_summed AS (
        ...
        FROM
            ...
            ,concrete
        WHERE
            payout_reference_dfk = concrete.invoice_number
        GROUP BY
            payout_reference_dfk
    )
    SELECT
        ...
        ,i.contact_name
        ,i.total
        ,sum(li.line_amount) FILTER (WHERE 
            li.account_code LIKE '53%' 
            OR
            li.account_code LIKE '55%'
            OR
            li.account_code LIKE '57%'
    ...
    WHERE
        concrete.invoice_number = i.invoice_number
        /*
        AND
