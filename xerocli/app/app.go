...
// UpdateBankTransactionReference updates a single bank transaction's reference field.
// It performs a GET-before-POST check to prevent overwriting stale data.
func (a *App) UpdateBankTransactionReference(ctx context.Context, cfgPath, uuid, reference string) error {
	cfg, err := LoadConfig(cfgPath)
	if err != nil {
		return err
	}
	...
	log.Printf("Fetching local version of bank transaction %s", uuid)
	localUpdatedTime, err := dbConn.GetBankTransactionUpdatedTime(uuid)
	if err != nil {
		return fmt.Errorf("could not get local record for transaction %s. Please sync first. Error: %w", uuid, err)
	}

	log.Printf("Fetching remote version of bank transaction %s for concurrency check", uuid)
	remoteTx, err := xeroClient.GetBankTransactionByID(ctx, uuid)
	if err != nil {
		return fmt.Errorf("could not fetch remote bank transaction %s: %w", uuid, err)
	}

	// Concurrency check: fail if remote version is newer than our local one.
	if remoteTx.Updated.After(localUpdatedTime) {
		return fmt.Errorf("concurrency error: transaction %s has been updated in Xero since last sync. Please sync and try again", uuid)
	}

	log.Printf("Updating reference for transaction %s to '%s'", uuid, reference)
	updatedTx, err := xeroClient.UpdateBankTransactionReference(ctx, remoteTx, reference)
	if err != nil {
		return fmt.Errorf("failed to update bank transaction: %w", err)
	}

	log.Println("Upserting updated record into local database...")
	if err := dbConn.UpsertBankTransactions([]xero.BankTransaction{updatedTx}); err != nil {
		return fmt.Errorf("failed to upsert updated transaction: %w", err)
	}
